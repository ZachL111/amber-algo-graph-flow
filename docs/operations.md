...
`amber-algo-graph-flow` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for graph analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 151. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 4.
